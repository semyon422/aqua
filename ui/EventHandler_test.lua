...
---@param t testing.T
function test.bubble_phase(t)
	local event_handler = EventHandler()
	event_handler:registerEvent("mousePressed")
	...
	local window = root:add(Node({ id = "window", mousePressed = f }))
	local random_thing = window:add(Node({ id = "random_thing", mousePressed = f }))
	local section = window:add(Node({ id = "section", mousePressed = f }))
	local button = section:add(Node({ id = "button", mousePressed = f }))

	event_handler:dispatchEvent(Event("mousePressed", button))
	t:teq(receivers, { button, section, window })
end

---@param t testing.T
function test.song_select(t)
	local event_handler = EventHandler()
	event_handler:registerEvent("mouseClick")
	...
	local song_select = root:add(Node({
		mouseClick = function(self, e)
			if e.data == "start_game" then
				game_started = true
			end
		end
	}))

	local play_button = song_select:add(Node({
		mouseClick = function(self, e)
			e.data = "start_game"
		end
	}))

	event_handler:dispatchEvent(Event("mouseClick", play_button))
	t:eq(game_started, true)

	--------------------
	game_started = false

	local chart_list = song_select:add(Node())
	local chart_panel = chart_list:add(Node({
		mouseClick = function(self, e)
			e.data = "start_game"
		end
	}))
