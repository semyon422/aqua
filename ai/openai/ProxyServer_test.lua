...
function test.streams_preserved_completion_finish_reason(t)
	...
	t:eq(response.status, 200)
	local role_position = assert(response.body:find('"role":"assistant"', 1, true))
	local content_position = assert(response.body:find('"content":"Partial"', 1, true))
	t:assert(role_position < content_position)
