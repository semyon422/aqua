...
- The proxy translates `prompt_cache_key`, `tool_choice`, and `response_format` to their Responses equivalents. A client-provided cache key is forwarded unchanged; when absent, the upstream field is omitted so automatic prefix caching remains available. Forced function choices are flattened to a Responses function selector, while Chat Completions JSON-schema formats become `text.format` objects.
