...
function test.client_to_server(t)
	...
	t:assert(client:send("text", "helloworld"))
	t:assert(client:send("text", long_msg_1))
	t:assert(client:send("text", long_msg_2))
	t:assert(client:send("binary", "\0\1\2\3"))
	t:assert(client:send("ping", "ping msg"))
	t:assert(client:send("pong", "pong msg"))
