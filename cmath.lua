...
---@class complex: ffi.cdata*, integer
---@operator unm: complex
---@operator pow: complex
...
local function tocomplex(v) return iscomplex(v) and v or new(v) end
...
function cmath.frompolar(r, t) return new(r * math.cos(t), r * math.sin(t)) end
function cmath.random() return cmath.frompolar(math.random(), math.random() * 2 * math.pi) end

function cplex.exp(z) return new(math.exp(z[0]) * math.cos(z[1]), math.exp(z[0]) * math.sin(z[1])) end
function cplex.log(z, k) return z == 0 and 0 / 0 or new(math.log(#z), z:arg() + 2 * math.pi * assert_int(k or 0)) end
function cplex.pow(z, n, k) return z == 0 and n ~= 0 and 0 or (n * z:log(k)):exp() end
function cplex.copy(z) return new(z[0], z[1]) end
...
function cplex.conj(z) return new(z[0], -z[1]) end
...
function mt.__add(z, c) return new(z[0] + c[0], z[1] + c[1]) end
function mt.__sub(z, c) return new(z[0] - c[0], z[1] - c[1]) end
function mt.__mul(z, c) return new(z[0] * c[0] - z[1] * c[1], z[0] * c[1] + z[1] * c[0]) end
function mt.__div(z, c) return new((z[0] * c[0] + z[1] * c[1]) / c:abs2(), (z[1] * c[0] - z[0] * c[1]) / c:abs2()) end
...
function mt.__unm(z) return new(-z[0], -z[1]) end
